...
//possible way:首先遍历邻居节点，记录下受破产影响的节点，放入到数据结构中，建议考虑队列（先进先出）
//数组不可行，因为数组是固定大小，队列可能需要自己实现，可以参照https://gist.github.com/mycodeschool/7510222
//然后while循环，数组不空的情况下取出队列当中第一个，遍历neighbor
//找出受到影响的节点，放入队列当中。在碰到已经破产的节点时直接跳过，如此循环直到队列为空，意味着所有节点都已经被处理过
//如此可以得到有多少个节点会受到影响。
//不建议用递归，这个递归层数太深，而且很容易出现内存不够的问题，终止条件不明显，较难理解。
